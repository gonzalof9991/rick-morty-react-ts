...
export default function Card({schema, item, classes}: CardProps) {
    const renderImage = () => {
        if (schema !== 'character') {
            return null;
        }
    ...
    return (
        <li className={'dark:bg-gray-800 flex flex-col gap-x-2 shadow cursor-pointer transition-all ease-linear hover:scale-105 md:flex-row ' + classes
        }>
            {
                renderImage()
            }
